...
while True:
    ...
    try:
        while True:
            ...
            print(len(position_vectors))

            # Select the correspoding index and Flatten to 1d vectors
            flat_list = [coord for row in position_vectors for coord in row]
            
            if len(flat_list) != 26 * 3:
                raise ValueError("flat_list length is not 78.")
            ...
            response_encode = gesture_server_socket.recv(1024)
            response = response_encode.decode()
            print("Received from gestures:", response)
